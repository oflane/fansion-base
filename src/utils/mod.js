 ...
 * @param handle 依赖处理方法
 */
export const depend = (comp, deps, handle = defaultDependHandle) => () => Promise.all([comp, ...deps].map(d => typeof d === 'function' ? d() : typeof d === 'string' ? createRequest(d) : d)).then(handle)
